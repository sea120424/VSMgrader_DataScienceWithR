...
def load_csv(path, dictionary, number):
    fp = open(path)
    line = fp.readline()
    ...
    while line:
        ...
        dictionary[element[0]] = {'tf': float(element[1]) + b, 'idf': math.log(number / (float(element[2]) + 1), 10) }
...
def main():
    ...
    load_csv(beginner, beginner_dict, beginner_num)
    load_csv(native, native_dict, native_num)
    load_csv(professional, professional_dict, professional_num)
    load_csv(medium, medium_dict, medium_num)
